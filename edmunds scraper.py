...
def handle_req(url, download=False):
   req = Request(url, headers={'User-Agent':str(user_agent_rotator.get_random_user_agent()),
...
def get_pictures_from_alixpress(url_input):
   ...
   for i in range(1, len(img_url_list)+1):
      ...
      folderpath = os.path.join(config.DROPSHIP_DIR, filename_base)
      filename = os.path.join(folderpath,filename_base+'-'+str(i)+'.jpg')
      

      path = Path(folderpath)
      if os.path.exists(path.parent):
         config.mkdir_if_not_exist(path.parent)
      elif not os.path.exists(folderpath):
         config.mkdir_if_not_exist(folderpath)
...
while True:
   ...
   handle_req(url_in, download=True)
